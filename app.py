...
from flask import Flask, render_template, request, jsonify
    ...
    # render_template help to renders our HTML templates dynamically. 
    # request handles query parameters in search requests (from user), jsonify converts database query results into JSON format for API responses.
    # jsonify help us converts data into JSON format for the API responses.
import sqlite3
    # Connects to the SQLite database to retrieve genetic data stored there.
import requests
    # requests: Fetches external data (i.e., UniProt API).
import pandas as pd
    # Processes database query results for statistical analysis.

# Creates the Flask web application & specifies our static files (CSS, JS) directory.
app = Flask(__name__, static_folder="static")


# Search Q Function, housess all our database interaction: 
# opens a connection, executes a query, fetches results, and then closes the connection.
def query_database(query, params=()):
    """Helper function to query the database."""
    conn = sqlite3.connect("genetics.db")   # Connect to the database
    cursor = conn.cursor()  # Creates a 'cursor' to execute SQL queries - what is that??
...
def get_db_connection():
    ...
    return sqlite3.connect("genetics.db")  # Connects to SQLite database

# Homepage Route: 
# Loads index.html, where the user can enter search queries.
@app.route("/")
def index():
...
# Search Results Page: Loads results.html, which displays search results dynamically w/ render_template.
@app.route("/results")
def results():
...
def gene_page(gene_name):
    ...
    uniprot_data = None
    # Also queries UniProt API if the gene is missing from the gene info database.
    if not db_data:
        uniprot_api_url = f"https://rest.uniprot.org/uniprotkb/search?query=gene:{gene_name}+AND+organism_id:9606&format=json" # organism_id ensures human only
        try:
...
# Retrieves population details (genetic diversity, disease traits, sampling locations).
...
def population_page(population_name):
    ...
    ihs_statistics = None  # ✅ Initialize ihs_statistics to avoid UnboundLocalError
    ...
    # Retrieve FST Mean and Std if the population is SA
    fst_statistics = None
    if population_name == "SA":
        fst_results = query_database(
            """
            SELECT mean, std FROM mean_std_SA WHERE stat = 'fst'
            """
        )
        if fst_results:
            fst_statistics = {
                "mean_fst": fst_results[0][0],
                "std_fst": fst_results[0][1]
            }

        # Retrieve iHS statistics for all chromosomes if the population is SA
        ihs_results = query_database(
            """
            SELECT stat, mean, std FROM mean_std_SA WHERE stat LIKE 'ihs_chr%'
            """
        )
        if ihs_results:
            ihs_statistics = [
                {"chromosome": result[0].replace("ihs_chr", ""), "mean": result[1], "std": result[2]}
                for result in ihs_results
            ]
    ...
    return render_template(
        ...
        fst_statistics=fst_statistics,  # Pass FST data
        ihs_statistics=ihs_statistics  # Pass iHS data (now always defined)
    )
...
# Returns details for a given gene.
...
def gene_details(gene_name):
    ...
    """, (gene_name,))
    return jsonify(results)
# Retrieves information for a specific population.
...
# Retrieves all available (distinct/unique) chromosomes from the ihs_stat_SA table.
...
# Retrieves all unique sub-populations from ihs_stat_SA.
...
# Fetches iHS selection statistics for a specific chromosome and optional sub-population.
...
def get_ihs_values():
    ...
    # Ensures chromosome is provided, else returns an error.
    if not chromosome:
        return jsonify({"error": "Chromosome parameter is required"}), 400
    
    table_name = "ihs_stat_SA"
    # Filters by chromosome and optionally by population.
    query = f"""
...
# Fetches FST values for population differentiation analysis.
...
def get_fst_values():
    ...
    # Filters data by chromosome (sorted by position)
    query = f"""
...
# Fetches FST comparison data between populations.
...
def get_fst_data():
    ...
    try:
        ...
        query = f"SELECT CHROM as Chromosome, POS as Position, dbSNP as SNP, FST FROM {population_comparison}"
...
def fetch_ihs_data():
    ...
    query = "SELECT Chromosome, Position, iHS_Score, Population FROM ihs_stat_SA"
...
# Retrieves available chromosomes for FST population comparison.
@app.route("/populations", methods=["GET", "POST"])
def population_fst_plot():
    conn = sqlite3.connect('genetics.db')
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT CHROM FROM fst_GIHvsGBR")
    ...
    return jsonify({"chromosomes":available_chromosomes})
    
# Fetches iHS selection data, as a Pandas DataFrame.
@app.route("/fst_data")
def fetch_fst_data():
    conn = sqlite3.connect('genetics.db')
    cursor = conn.cursor()
...
# Provides the Overall FST and IHS statistics.
@app.route("/api/summary_stats")
def get_summary_stats():
    """Fetches overall FST and iHS statistics from mean_std_SA table."""

    # Fetch Overall FST Stats
    fst_stats = query_database(
        "SELECT mean, std FROM mean_std_SA WHERE stat = 'fst'"
    )
    fst_data = {"mean": fst_stats[0][0], "std": fst_stats[0][1]} if fst_stats else None

    # Fetch Overall iHS Stats (all chromosomes)
    ihs_stats = query_database(
        "SELECT stat, mean, std FROM mean_std_SA WHERE stat LIKE 'ihs_chr%'"
    )
    ihs_data = [
        {"chromosome": stat.replace("ihs_chr", ""), "mean": mean, "std": std}
        for stat, mean, std in ihs_stats
    ] if ihs_stats else None

    return jsonify({"fst": fst_data, "ihs": ihs_data})
